...
1. **空行也需要填充空格**——空字符串视觉宽度为 0，需填充 `slot_w` 个空格
2. **缩进内容无需特殊处理**——同样按视觉宽度计算即可
3. **content_w 选择**：设为 `None` 自动适配，或手动指定为最长行视觉宽度 + 2
4. **垂直连接自动对齐宽度**——`connect_boxes_vertical` 会将所有方框扩展到最大宽度
5. **纯英文内容**：直接用固定宽度字符串格式化即可，不需要 `visual_width`
6. **表格外边框**：使用 `┌┬┐`、`├┼┤`、`└┴┘` 等组合字符实现多列分隔
